...
def main_menu_keyboard():
    return InlineKeyboardMarkup(
        [[
         ...
         [
             InlineKeyboardButton("🛒 Товары в наличии",
                                  callback_data="available")
         ], [InlineKeyboardButton("📩 Связаться с продавцом",
                                  url=CONTACT_URL)]])
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(
        f"🟢 Кнопка нажата: `{query.data}` от `@{user.username}` (ID: {user.id})"
    )
    await query.answer()
    ...
    # 🔘 Товары по категории
        ...
        product_buttons = [[InlineKeyboardButton(p["name"], callback_data=k)]
                           for k, p in products.items()
                           if p.get("category") == cat_key]
        if not product_buttons:
            product_buttons = [[
                InlineKeyboardButton("❌ Нет товаров в этой категории",
                                     callback_data="available")
            ]]
        nav_buttons = [[
    ...
    # 🔘 Конкретный товар
        ...
        if not product.get("photos"):
            ...
            sent = await context.bot.send_media_group(
                chat_id=query.message.chat.id, media=media)
    ...
    # 🔘 Гарантия и отказ
        ...
        await query.edit_message_text(warranty_text,
                                      reply_markup=default_nav_keyboard(),
                                      parse_mode="HTML")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"⛔ Ошибка при запуске бота через {uptime:.2f} секунд: {type(e).__name__}: {e}"
        )
        traceback.print_exc()
        time.sleep(10)
